...
class Ant(Env):
    @classmethod
    def class_init(cls):
        cls.host_model = mujoco.MjModel.from_xml_path("assets/ant/ant.xml")
    ...
    def init(cls):
        ...
        init_qpos = jnp.zeros(15).at[2].set(0.75).at[3].set(1.0)
        data = data.replace(qpos=init_qpos)

        return cls.make_state(data)

    @classmethod
    def get_config(cls, substep=4):
    ...
    def host_render_frame(cls, state):
        ...
        cls.renderer.update_scene(host_data, "track")
